...
class STRIFE:
    ...
    def __init__(self, args):
        ...
        assert args.model_type in [0, 1, 2, 3,4,5], 'Please provide a valid setting for generating molecules: 0, 1 or 2. See the "Running STRIFE" section of the github readme for more details'
        ...
        if bool(args.load_specified_pharms):
            ...
            if len(glob.glob(f'{self.hotspotsLoc}/acceptorHotspot.sdf')) + len(glob.glob(f'{self.hotspotsLoc}/donorHotspot.sdf')) == 0:
                ...
                hotspotsDict['Donor'] = Chem.SDMolSupplier(f'{self.hotspotsLoc}/donorHotspot.sdf')[0]
            elif len(glob.glob(f'{self.hotspotsLoc}/donorHotspot.sdf')) == 0:
                hotspotsDict = {}
                hotspotsDict['Acceptor'] = Chem.SDMolSupplier(f'{self.hotspotsLoc}/acceptorHotspot.sdf')[0]
        ...
        fc, self.evI, evp, fc2 = self.preprocessing.preprocessFragment(fragSmiles, fragMol3D)
        ...
        if args.model_type == 0:
            ...
            self.HotspotsDF = self.HotspotsDF.head(1)
        elif args.model_type == 4 or args.model_type == 5:
            print('model type is 4')
                        #Set up the hotspotsDF
            self.HPositions = [] #list for hotspot positions
            self.HType = [] #list for hotspot type (donor/acceptor)
            self.Distances = [] #Distance from exit vector
            ...
            print('something up with hotspotsDF line')
            self.HotspotsDF = pd.DataFrame({'distFromExit':self.Distances, 'angFromExit':self.Angles, 'position':self.HPositions, 'type':self.HType, 'score':self.Scores}).sort_values('score', ascending = False).reset_index(drop=True)
            print(self.HotspotsDF)
            print(f'hotspot we are elaborating to has index {args.hotspot_index}')
            self.HotspotsDF = pd.DataFrame(self.HotspotsDF.loc[args.hotspot_index]).T.reset_index(drop=True)
            print(self.HotspotsDF)
        elif args.model_type == 3:
            print('model type is 3')
            print(hotspotsDict)
            #Set up the hotspotsDF
            self.HPositions = [] #list for hotspot positions
            self.HType = [] #list for hotspot type (donor/acceptor)
            ...
            self.Plausibility = []
            for pharm in ['Acceptor', 'Donor']:
                print('hotspotsDict[pharm].GetNumHeavyAtoms()')
                print(hotspotsDict[pharm].GetNumHeavyAtoms())
                if hotspotsDict[pharm].GetNumHeavyAtoms() > 0: #Changed self.hotspotsDict to hotspotsDict here
                    print('beginning of hotspotsDF loop')
                    self.scoresList.append(np.array((hotspotsDict[pharm].GetProp('vdw').split())))
                    print(self.scoresList)
                    for atom in hotspotsDict[pharm].GetAtoms():
                        print(atom)
                        pos = np.array(hotspotsDict[pharm].GetConformer().GetAtomPosition(atom.GetIdx()))
                        exitVectorNeighbor = np.array(self.fragCore.GetConformer().GetAtomPosition(self.fragCore.GetAtoms()[self.evI].GetNeighbors()[0].GetIdx()))
                        plaus = self.preprocessing.plausibilityScore(pos, self.exitVectorPos,exitVectorNeighbor)

                        self.Plausibility.append(plaus)
                        self.HPositions.append(pos)
                        self.Distances.append(self.preprocessing.vectorDistance(pos, self.exitVectorPos))
                        self.Angles.append(self.preprocessing.vectorAngle(pos, self.exitVectorPos))
                        self.HType.append(pharm)
            ...
            for sublist in self.scoresList:
                for score in sublist:
                    self.Scores.append(float(score))
            print('just before setting up df')
            print(len(self.Distances))
            print(len(self.Angles))
            print(len(self.HPositions))
            print(len(self.HType))
            print((self.Plausibility))
            print(self.Scores)
            self.HotspotsDF = pd.DataFrame({'distFromExit':self.Distances, 'angFromExit':self.Angles,'position':self.HPositions, 'type':self.HType, 'score': self.Scores , 'plausibility':self.Plausibility}).sort_values('score', ascending = False).reset_index(drop = True)
            print(self.HotspotsDF)
            self.HotspotsDF = self.HotspotsDF[self.HotspotsDF['plausibility'] == 1].head(1).reset_index(drop=True)
            print('hotspot to elaborate to is...')
            print(self.HotspotsDF)
            print('evp')
            print(self.exitVectorPos)




        elif args.model_type == 1:
            #for the new hotspots (hotspotsInProtein = True)
            #uses top scorer
            #Set up the hotspotsDF
            self.HPositions = [] #list for hotspot positions
            self.HType = [] #list for hotspot type (donor/acceptor)
            ...
            self.HotspotsDF = pd.DataFrame({'distFromExit':self.Distances, 'angFromExit':self.Angles,'position':self.HPositions, 'type':self.HType, 'score': self.Scores , 'plausibility':self.Plausibility}).sort_values('distFromExit', ascending = True).reset_index(drop = True)
            self.HotspotsDF = self.HotspotsDF[self.HotspotsDF['plausibility'] == 1].head(1).reset_index(drop=True)
            #self.HotspotsDF = self.HotspotsDF.head(1) 
           #self.HotspotsDF = self.HotspotsDF[self.HotspotsDF['plausibility'] == 1].head(1).reset_index(drop=True)
    ...
    def run(self, args):
        for line in open('/proc/self/status'):
            if 'Cpu' in line:
                print(line)
       	        print('in run statement')
        print('!!!!!!!!!!!!!!!!!')
        #output satisfied hotspots sdf

        molPharmProf = Chem.MolFromSmiles('I')
        conf = Chem.Conformer(molPharmProf.GetNumAtoms())
        conf.SetAtomPosition(0, self.HotspotsDF['position'][0])
        conf.SetId(0)
        molPharmProf.AddConformer(conf)
        Chem.MolToMolFile(molPharmProf,f'{self.storeLoc}/satisfiedHotspots.sdf')
        #Function which runs a version of the STRIFE algorithm depending on the specified model_type
        if args.model_type == 0:
            print('model type is 0')
        ...
        if args.model_type == 1:
            ...
            for index in range(len(self.HotspotsDF)):
                self.HotspotsDF['distFromExit'][index] = self.HotspotsDF['distFromExit'][index]-2.5
            print('elaborate to....')
            print(self.HotspotsDF)
            if self.HotspotsDF['distFromExit'][index] <= 0:
                raise Exception("boooo, negative distance")
            self.hSingles = self.preprocessing.prepareProfiles(self.HotspotsDF)
            self.hMulti = self.preprocessing.prepareProfiles(self.HotspotsDF, single = False) #For satisfying multiple pharmacophoric point simultaneously
            print(self.HotspotsDF)
        ...
        elif args.model_type == 3 or args.model_type == 4:
            self.hotspotsInProtein = False
            print('fine up to here')
            self.hSingles = self.preprocessing.prepareProfiles(self.HotspotsDF)
            #self.hMulti = self.preprocessing.prepareProfiles(self.HotspotsDF, single = False) #For satisfying multiple pharmacophoric point simultaneously
            print('hotspotsinprotein')
            print(self.hotspotsInProtein)
            print(self.HotspotsDF)
            print(self.hSingles)
            self.runWholePipeline(totalNumElabs = args.number_elaborations, numElabsPerPoint = args.number_elaborations_exploration, hotspotsInProtein = self.hotspotsInProtein)
        else:
    ...
    def runWholePipeline(self, totalNumElabs = 250, numElabsPerPoint = 250, hotspotsInProtein = False):
        ...
        if self.HotspotsDF.shape[0] > 0:
            ...
            self.identifyQuasiActives(hotspotsInProtein)
    ...
    def exploration(self, numElabsPerPoint = 250, n_cores = None):
        ...
        for k in self.hSingles.keys():
            ...
            for line in open('/proc/self/status'):
                if 'Cpu' in line:
                    print(line)
                    print('immediately before docking')
            print('!!!!!!!!!!!!!!!!!')
            p = psutil.Process()
            all_cpus = list(range(psutil.cpu_count()))
            p.cpu_affinity(all_cpus)
    ...
    def identifyQuasiActives(self,hotspotsInProtein=False):
        ...
        print(self.singleDistances)
        for k in self.hSingles.keys():
            #Select up to 5 molecules where the pharmaocphore is within 2A of the pharmacophoric point
            if hotspotsInProtein == False:
                print('checkaroo buckaroo')
                #NOTE: this was previously 2 have only just changed it to 1!!!!!
                self.singleQuasiActives[k] = self.singleDistances[k].loc[self.singleDistances[k]['distance'] < 1].drop_duplicates('smiles').head(5)
            elif hotspotsInProtein == True:
                print(self.singleDistances[k])
                self.singleQuasiActives[k] = self.singleDistances[k].loc[self.singleDistances[k]['distance'] < 3.5].drop_duplicates('smiles').head(5)
            print('!!!! 2nd')
            print(self.singleQuasiActives[k])
    ...
    def refinement(self, totalNumElabs = 250, n_cores = None):
        ...
        if totalNumElabs is None:
            ...
            try:
                ...
                #no elabs made that match the profile
                return 0
...
if __name__=='__main__':
    ...
    parser.add_argument('--hotspot_index', type = int, default = 1, 
            help='Number of CPU cores to use for docking and other computations. Specifiying -1 will use all available cores')
    #TODO
    ...
    original_output_dir = arguments.output_directory
    for indx in range(len(fragments)):
        indx = indx
        smi = smiles[0][indx]

        arguments.output_directory = '%s/file%s'%(original_output_dir, indx)
        if not os.path.exists(arguments.output_directory):
            os.makedirs(arguments.output_directory)

        w = Chem.SDWriter(f'{arguments.output_directory}/frag.sdf')
        w.write(fragments[indx])
        w.close()
        arguments.fragment_sdf = f'{arguments.output_directory}/frag.sdf'
